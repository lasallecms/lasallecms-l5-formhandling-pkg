...
class UpdateCommand extends Command implements SelfHandling
{
    ...
    public function handle()
    {
        ...
        $newUpdateCommand->field_list = $field_list;

        // add crud_action to the new command instance
        $newUpdateCommand->crud_action = Input::get('crud_action');
        ...
        $classname_formprocessor_update = $this->getClassnameFormprocessorUpdate();
        ...
        $this->$classname_formprocessor_update = $this->app->make( $namespace_formprocessor."\\".$classname_formprocessor_update );


        // Now onto the Form Processor, aka "The Command Handler"
        return $this->$classname_formprocessor_update->quarterback($newUpdateCommand);
    }
}